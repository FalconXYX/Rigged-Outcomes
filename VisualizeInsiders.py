...
def load_data(keyword=None):
    ...
    if keyword:
        markets = [m for m in markets if keyword.lower() in m["name"].lower() or keyword.lower() in m["slug"].lower()]
    ...
    m = markets[0]
    insider_csv = f"data/{m['output']}_insiders.csv"
    
    if not os.path.exists(insider_csv):
        print(f"Data missing. Run the scraper for {m['name']} first.")
        sys.exit(1)
    ...
    market_title = get_market_title(m, df)
...
def calculate_network_metrics(df):
    ...
    df["theta_rad"] = df["user_id"].apply(lambda x: int(x[2:8], 16) % 360) * (np.pi / 180)
...
def fig_threat_constellation(df):
    cat_colors = {
        "Normal Whale": TEAL,
        "Suspicious (New Account({} Trades))".format(FIRST_X_TRADES): BURNT_PEACH,
        "Suspicious (>{}% Concentration)".format(PERCENT_VOLUME): HIGH,
    ...
    for cat, color in cat_colors.items():
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=(
                ...
                "<b>Prior Bets:</b> %{customdata[1]}<br>"
                "<b>Concentration:</b> %{customdata[2]:.1f}%<br>"
                "<b>Position Size:</b> $%{customdata[3]:,.0f}<extra></extra>"
            )
        ))
    ...
    fig.add_trace(go.Scatter(
        ...
        textfont=dict(color=JET_BLACK, size=12),
        hoverinfo="skip",
        showlegend=False
    ))
    ...
    fig.add_annotation(
        x=15, y=15,
        text="<b>Core Threat Cluster</b><br>(High Concentration, Zero History)",
        showarrow=True, arrowhead=2, arrowsize=1, arrowwidth=2, arrowcolor=RED,
        ax=40, ay=40, font=dict(color=RED, size=13),
        bgcolor="rgba(255,255,255,0.85)", borderpad=4
    )
    
    fig.add_annotation(
        x=-80, y=-80,
        text="<b>Normal Orbit</b><br>(Established Traders)",
        showarrow=True, arrowhead=2, arrowsize=1, arrowwidth=1.5, arrowcolor=TEAL,
        ax=-40, ay=-40, font=dict(color=TEAL, size=13),
        bgcolor="rgba(255,255,255,0.85)", borderpad=4
    )
...
def fig_treemap_concentration(df):
    ...
    fig = go.Figure(go.Treemap(
        ...
        hovertemplate="<b>%{label}</b><br>Position: $%{value:,.0f}<extra></extra>"
    ))
...
def run_app(keyword=None, port=5003):
    df, market_title = load_data(keyword)
    ...
    app = Dash(__name__, title="Insider Radar")
    ...
    print(f"\nLaunching Interactive Threat Radar for: {market_title}")
